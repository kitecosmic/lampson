...
async function loadPorts() {
  ...
  for (const p of list) {
    ...
    const kb = d.querySelector('button'); kb.onclick = () => { const holder = document.createElement('span'); holder.className = 'del'; kb.replaceWith(holder); inlineConfirm(holder, `¿matar pid ${p.pid}?`, async () => { await api('/api/ports/kill', { pid: p.pid }); loadPorts(); }); setTimeout(() => { if (!holder.classList.contains('ask')) loadPorts(); }, 6100); };
    box.appendChild(d);
  }
}
async function loadProcs() {
  ...
  for (const p of list) {
    ...
    const b = d.querySelector('button'); if (b) b.onclick = async (ev) => { ev.stopPropagation(); await api('/api/proc/stop', { name: p.name }); loadProcs(); if (procOpen === p.name) openProc(p.name); };
    box.appendChild(d);
  }
}
