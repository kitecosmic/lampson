...
async function loadApprovals() {
  let r; try { r = await (await fetch('/api/approvals')).json(); } catch (e) { return; }
  ...
  for (const a of list) {
    ...
    d.querySelectorAll('button').forEach(b => b.onclick = async () => { d.querySelectorAll('button').forEach(x => x.disabled = true); await api('/api/approve', { id: a.id, decision: b.dataset.ok === '1' }); setTimeout(loadApprovals, 600); });
    box.appendChild(d);
  }
  ...
  for (const a of pend) {
    ...
    el.dataset.id = a.id; el.querySelectorAll('button').forEach(b => b.onclick = async () => { el.querySelectorAll('button').forEach(x => x.disabled = true); await api('/api/approve', { id: a.id, decision: b.dataset.ok === '1' }); });
  }
