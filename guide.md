...
Everything the [README](README.md) leaves out: how Lampson runs, every feature in detail, the architecture, and
the notes on the Synsema runtime that shaped the code. Start with the README if you have not installed it yet;
the short version of these pages also lives at [lampson.org/docs](https://lampson.org/docs).
...
shared by both UIs. Change it any time: `/model` in the terminal (provider → model listed from the API → key if missing; `/model glm glm-5.3-flash` goes direct; `/provider` and `/setup` are aliases), or click
